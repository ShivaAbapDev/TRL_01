...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  METHOD CalculateTravelID.
    ...
    READ ENTITIES OF zi_rap_travel_GSP IN LOCAL MODE
      ENTITY Travel
    ...
    SELECT SINGLE
        FROM  zrap_travel_gsp
        FIELDS MAX( travel_id ) AS travelID
        INTO @DATA(max_travelid).

    " Set the travel ID
    MODIFY ENTITIES OF zi_rap_travel_GSP IN LOCAL MODE
    ENTITY Travel
      UPDATE
      ...
      IF travel-AgencyID IS INITIAL OR NOT line_exists( agencies_db[ agency_id = travel-AgencyID ] ).
        ...
        APPEND VALUE #( %tky        = travel-%tky
                        ...
                        %msg        = NEW zcm_rap_GSP(
                                          ...
                                          textid   = zcm_rap_GSP=>agency_unknown
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-CustomerID IS INITIAL OR NOT line_exists( customers_db[ customer_id = travel-CustomerID ] ).
        ...
        APPEND VALUE #(  %tky        = travel-%tky
                         ...
                         %msg        = NEW zcm_rap_GSP(
                                           severity   = if_abap_behv_message=>severity-error
                                           textid     = zcm_rap_GSP=>customer_unknown
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-EndDate < travel-BeginDate.
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_rap_GSP(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_rap_GSP=>date_interval
      ...
      ELSEIF travel-BeginDate < cl_abap_context_info=>get_system_date( ).
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_rap_GSP(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_rap_GSP=>begin_date_before_system_date
                                                 begindate = travel-BeginDate )
    ...
    result =
      VALUE #(
        FOR travel IN travels
          LET is_accepted =   COND #( WHEN travel-OverallStatus = Overall_status-accepted
                                      THEN if_abap_behv=>fc-o-disabled
                                      ELSE if_abap_behv=>fc-o-enabled  )
              is_rejected =   COND #( WHEN travel-OverallStatus = Overall_status-canceled
                                      THEN if_abap_behv=>fc-o-disabled
                                      ELSE if_abap_behv=>fc-o-enabled )
          IN
            ( %tky                 = travel-%tky
...
*   For the Travel Status we need the before-image from the database. We perform this for active (is_draft=00) as well as for drafts (is_draft=01) as we can't distinguish between edit or new drafts
      ...
      IF is_update_requested = abap_true.
        ...
        IF has_before_image = abap_true.
          ...
          IF update_granted = abap_false.
            APPEND VALUE #( %tky        = travel-%tky
                            %msg        = NEW zcm_rap_GSP( severity = if_abap_behv_message=>severity-error
                                                            textid   = zcm_rap_GSP=>unauthorized )
                          ) TO reported-travel.
          ENDIF.
  ...
  METHOD is_update_granted.
    IF has_before_image = abap_true.
      AUTHORITY-CHECK OBJECT 'ZOSTATGSP'
        ID 'ZOSTATGSP' FIELD Overall_status
        ID 'ACTVT' FIELD '02'.
    ELSE.
      AUTHORITY-CHECK OBJECT 'ZOSTATGSP'
        ID 'ZOSTATGSP' DUMMY
  ...
  METHOD is_delete_granted.
    IF has_before_image = abap_true.
      AUTHORITY-CHECK OBJECT 'ZAOSTATGSP'
        ID 'ZAFSTATGSP' FIELD Overall_status
        ID 'ACTVT' FIELD '06'.
    ELSE.
      AUTHORITY-CHECK OBJECT 'ZAOSTATGSP'
        ID 'ZAFSTATGSP' DUMMY
